...
// 环节 1：成员起始位置的对齐（「局部对齐」） 每个成员的起始地址必须是「自身对齐值」（或 pack 限制后的有效对齐值）的整数倍：
// 环节 2：结构体总大小的对齐（「整体对齐」） 结构体的总大小必须是「所有成员对齐值的最大值」（或 pack 限制后的最大值）的整数倍 
...
// 对齐规则：
// 成员的有效对齐值 = min (成员自身对齐值，pack 指定值 n)；
...
//
// 类型 / 概念	32 位环境	64 位环境	核心原因
// 普通指针（void*/int * 等）	4 字节	8 字节	指针存的是内存地址，32 位地址总线最多表示232个地址（4 字节），64 位同理
// size_t	4 字节	8 字节	表示 “内存大小 / 数组索引” 的无符号类型，和指针位数对齐（避免地址越界）
// ptrdiff_t	4 字节	8 字节	表示两个指针的差值，本质是 “地址差”，必须和指针位数匹配
